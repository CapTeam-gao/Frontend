...
2. **팀 채팅 — 메시지 고정: 실구현 완료(8/6)**, 읽음 상태/담당 업무 표시는 아직 미착수. 디자인 목업(`Design/TeamChat.html`)의 고정 바를 다시 다듬어서(민트 배경 전체 채움 → 흰 배경+좌측 얇은 강조선으로 절제, 클릭 시 원본 메시지로 스크롤+하이라이트) `ChatMessage.jsx`/`ChatMessageList.jsx`/`UserTeamChat.jsx`/신규 `ChatPinnedBar.jsx`에 실제로 연결함. **백엔드에 고정 메시지 API가 아직 없어서 채널 전환 시 초기화되는 클라이언트 로컬 state로만 구현**(서버에 저장되지 않고, 다른 사람 화면에는 안 보임) — API가 확정되면 이 state를 서버 값으로 교체하면 됨. 읽음 인원 수 표시·팀원 담당 업무 필드는 여전히 백엔드 스펙 미확인 상태라 미착수. P2.
