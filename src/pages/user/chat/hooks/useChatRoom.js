...
const useChatRoom = ({ setError }) => {
    ...
    const clearChannelUnreadCount = (channelId) => {
    ...
    const increaseChannelUnreadCount = (channelId, receivedMessage) => {
        setChannelSummaries((prevSummaries) =>
            prevSummaries.map((summary) =>
                String(summary.channel?.id) === String(channelId)
                    ? {
    ...
    const handleSubmitChannelModal = async () => {
        ...
        try {
            ...
            if (channelModalMode === "edit") {
                const updatedChannel = await requestUpdateChatChannel(
                    targetChannel.id,
                    trimmedChannelName
                );
        ...
        (event) => {
            ...
            if (event.type === "CHANNEL_DELETED" && event.channelId) {
                setRoom((prevRoom) => {
                    ...
                    const nextChannels = (prevRoom?.channels ?? []).filter(
                        (channel) =>
                            String(channel.id) !== String(event.channelId)
                    );
